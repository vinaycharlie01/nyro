...
func setupTestRedis(t *testing.T) (*redis.RedisStore, *miniredis.Miniredis) {
	...
	// Create real Redis client pointing to mock
...
func TestRedisStore_BasicOperations(t *testing.T) {
	...
	t.Run("Set and Get", func(t *testing.T) {
		...
		assert.Equal(t, float64(42), resultMap["count"]) // JSON numbers are float64
	...
	t.Run("DeleteMulti", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.True(t, exists) // del3 should still exist
	})
}
...
func TestRedisStore_DistributedLocking(t *testing.T) {
	...
	t.Run("Lock Expiration", func(t *testing.T) {
		...
		// Old lock value should not be able to release new lock
		err = store.ReleaseLock(ctx, key, lockValue)
		require.NoError(t, err) // Should not error, just won't release
	})
	...
	t.Run("Extend Lock", func(t *testing.T) {
		...
		// Extend lock with correct lock value
...
func TestRedisStore_CacheStampedePrevention(t *testing.T) {
	...
	t.Run("GetOrSetWithLock - Concurrent Requests (Cache Stampede)", func(t *testing.T) {
		...
		// Simulate 10 concurrent requests hitting the same cache key
		concurrency := 10
		var wg sync.WaitGroup
		...
		// CRITICAL: Loader should be called only ONCE despite 10 concurrent requests
		// This proves cache stampede prevention works!
...
func TestRedisStore_CustomConfiguration(t *testing.T) {
	t.Run("Custom Lock Configuration", func(t *testing.T) {
		// Create store with custom lock config
	...
	t.Run("Default Configuration", func(t *testing.T) {
		// Verify default config is applied when no options provided
		store, mockRedis := setupTestRedis(t)
		defer mockRedis.Close()
		defer store.Close()
